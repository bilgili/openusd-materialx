...
class BinaryBdistWheel(bdist_wheel):
    ...
    def get_tag(self):
        ...
        # The hosted macOS Python is universal2, but the bundled native libraries are built
        # single-arch (the runner's arch) by CMake. Retag universal2 -> the real arch so the
        # wheel matches its binaries; otherwise delocate fails ("Failed to find any binary
        # with the required architecture") and pip would mis-resolve the wheel.
